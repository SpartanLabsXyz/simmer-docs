...
| `dry_run=True` | Nothing — preview only | No position is created (except on Kalshi via the SDK — see below) |
...
A single-trade preview. The server validates the trade, calculates price/shares/cost, and returns the result without executing. No state changes, no order signed.
...
<Warning>
**Kalshi ignores `dry_run` through the Python SDK and places a real order.** `client.trade(venue="kalshi", dry_run=True)` reaches the Kalshi signing path before the flag is read. Tracked as SIM-5041. Preview Kalshi trades through the REST endpoint instead — it honours `dry_run` on every venue.
</Warning>

Two limits apply on every venue. A dry run is **not a permission check**: the server skips account trading-limit enforcement on previews, so a clean preview can still be rejected live for a daily buy cap, a spend cap, or a failed-trade cooldown. Read `client.get_settings()` for those. And on Polymarket the preview prices from the market's external price rather than the executable book, so neg-risk estimates can differ from the live fill.
