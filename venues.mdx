...
| `dry_run=True` | Nothing — preview only | No position is created on sim or Polymarket. **Not supported on Kalshi** — see below |
...
A single-trade preview on sim and Polymarket. The server validates the trade, calculates price/shares/cost, and returns the result without executing — no state changes, no order signed. Kalshi is the exception; see the warning below.
...
<Warning>
**`dry_run` does not work on Kalshi, by either route.** `client.trade(venue="kalshi", dry_run=True)` reaches the Kalshi signing path before the flag is read and **places a real order** (SIM-5041). `POST /api/sdk/trade` ignores the flag on Kalshi too, returning an error that points you back at the SDK.
