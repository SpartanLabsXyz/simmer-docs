...
|---|---|---|---|
| **Currency** | \$SIM (virtual) | pUSD (real, 1:1 with USDC.e) | USD (real) |
...
Real trading on Polymarket's orderbook. Requires a self-custody wallet with pUSD (the V2 collateral, 1:1 backed by USDC.e) on Polygon. If your wallet still holds USDC.e from pre-V2 Polymarket, migrate via [simmer.markets/dashboard](https://simmer.markets/dashboard) or the [V2 migration guide](/v2-migration).
...
2. **pUSD** on Polygon (the V2 collateral) — if you have USDC.e, wrap at [simmer.markets/dashboard](https://simmer.markets/dashboard)
