...
<Warning>
**Polymarket's Trading Contract (Polymarket Deposit Wallet) is a smart contract on Polygon.** Do not deposit funds from other chains, or other assets on Polygon except for USDC.e or pUSD — they will be unrecoverable.

This includes native POL: do **not** send POL to your Trading Contract. V2 trades are gasless via Polymarket's relayer, so the Trading Contract doesn't need POL — and POL sent to it cannot be moved out.

If you need POL for gas (one-time, for the V1 → V2 wrap), send it to your **owner wallet** — the EOA address you signed in with — not your Trading Contract.
</Warning>
...
The migration's approve + wrap + V2 approvals are gas-paid from your **owner wallet** (the EOA you signed in with — **not** your Trading Contract). Plan for ~0.5 POL on the owner wallet before you click Migrate. The dashboard shows the owner wallet address for top-up. **Do not send POL to your Trading Contract — it will be permanently stuck.**
...
<AccordionGroup>
  ...
  <Accordion title="Do I need POL (gas) to migrate?">
    Yes — the wrap requires an approve + wrap transaction pair, both paid in POL on Polygon. **This applies to both managed and external wallets** — Simmer signs the transactions on your behalf for managed wallets, but the gas still comes out of your **owner wallet's** POL balance (the EOA you signed in with — **not** the Trading Contract). The dashboard pre-flight requires at least **0.1 POL** before letting you click Migrate. If you don't have enough, the dashboard will display your owner wallet address so you can send POL from an exchange before retrying.

    **Important: do not send POL to your Trading Contract.** Polymarket's Trading Contract is a smart contract that has no native-asset withdrawal path — POL sent there cannot be moved out. POL only belongs on the owner wallet (EOA) for one-time migration gas. After migration, V2 trades are gasless via Polymarket's relayer.
  </Accordion>

  <Accordion title="What happens if I send funds to my Trading Contract on the wrong network or in the wrong asset?">
    The Trading Contract is a smart contract that only exists on Polygon. The same address on Base, Ethereum, Arbitrum, or any other chain is empty space, not your wallet — funds sent there cannot be recovered.

    On Polygon itself, the Trading Contract only has withdrawal paths for **USDC.e** and **pUSD**. Native POL, ETH, or arbitrary tokens sent to it cannot be moved out by you or by Simmer.
