...
The dashboard walks you through the applicable steps. For most users it's a **2-step flow**:
...
**Some accounts need a third step — Deposit Wallet upgrade.** Polymarket V2 routes all trades through a **Deposit Wallet** — a per-user smart contract that holds your collateral and signs trade batches. Accounts predating this requirement may receive `maker address not allowed` rejections from the Polymarket CLOB until they upgrade. The dashboard detects this automatically: when your EOA is blocked, an **"Upgrade to deposit wallet"** step appears between wrap and approvals. The upgrade deploys the smart contract on your behalf (managed wallet) or prompts a browser signature (external wallet). No funds leave your control during the upgrade.
...
4. **Deposit Wallet upgrade (if shown).** If the banner shows an **"Upgrade to deposit wallet"** step, click it — Simmer deploys the Deposit Wallet smart contract on your behalf. This is a one-time step for older accounts that Polymarket's V2 CLOB has flagged with `maker address not allowed`. Once deployed, the Deposit Wallet becomes your V2 trading address; funds move there automatically and V2 approvals apply to it.
...
**Stage 2 — Deposit Wallet upgrade (if shown):**

5. If the banner shows an **"Upgrade to deposit wallet"** step, your EOA has been flagged by Polymarket's V2 CLOB (`maker address not allowed`). Click **"Upgrade"** and sign the deploy transaction in your wallet. This deploys your personal Deposit Wallet smart contract on Polygon. The upgrade is a one-time step; once deployed, all V2 trades route through your Deposit Wallet.

**Stage 3 — Activate V2 trading:**

6. The banner switches to **"Activate V2 trading"** with an approval checklist
7. **Click "Activate Trading"** and sign **~8 more transactions**:
   - 4 pUSD approvals (one per V2 spender contract)
...
Total: ~10–11 wallet signatures (plus 1 more if the Deposit Wallet upgrade applies), ~1–2 minutes including transaction confirmations. Your **agent wallet** (the EOA — not the Deposit Wallet) needs at least **0.5 POL** for gas across all transactions.
