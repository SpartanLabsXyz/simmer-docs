...
2. **Click** the orange "Migrate to V2" banner at the top of the section
3. **Wait ~30–60 seconds** — Simmer signs and submits two on-chain transactions (approve + wrap) on your behalf. **No wallet popups, no extra clicks needed.**
...
Your wallet's POL covers the gas (~0.06 POL for the two transactions). If you don't have at least **0.1 POL** in your Polymarket wallet, the migration won't start — the dashboard shows your wallet address so you can send POL from an exchange first.

### External wallet (self-custody — MetaMask, Rabby, Coinbase Wallet, etc.)

1. **Go to** your Agent dashboard → **Portfolio → Polymarket** tab
2. **Make sure your trading wallet is the active account** in your browser wallet extension (this may differ from the wallet you used to log in to Simmer)
3. **Click** the orange "Migrate to V2" banner
4. **Sign the first transaction** in your wallet — `approve` USDC.e for the wrap contract
5. **Sign the second transaction** — `wrap` USDC.e to pUSD
6. **The banner turns green** — "Migrated to V2 — ready to trade"

Your wallet needs at least **0.1 POL** for gas across both transactions. Alternatively, you can migrate directly at [polymarket.com](https://polymarket.com) if you prefer their gasless flow.
