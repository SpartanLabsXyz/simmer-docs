...
Legacy direct-EOA accounts may still need POL for a one-time wrap. If the dashboard asks you for POL, send it to your **agent wallet** — the EOA Simmer signs trades from — not your Deposit Wallet.
</Warning>
...
- **What you need to do.** If you hold USDC.e in your Simmer Polymarket wallet, use the Polymarket wallet action before your next Polymarket trade. It converts USDC.e to pUSD and, for Deposit Wallet users, moves funds onto the Deposit Wallet where V2 trading happens.
...
The dashboard walks you through the applicable steps in the Polymarket wallet section. For active V2 trading, the normal path is:
...
If your account already has a Deposit Wallet, the dashboard uses it. If your account still trades directly from an older EOA, the dashboard may show an upgrade action before you can place V2 orders. The upgrade deploys the smart contract on your behalf (managed wallet) or prompts a browser signature (external wallet). No funds leave your control during the upgrade.
...
### Managed wallet with a Deposit Wallet (default for most users)

This is the current default funding and trading path for anyone who hasn't explicitly set up an external wallet on Simmer. Your Deposit Wallet is the address that holds Polymarket collateral and places V2 trades; the agent wallet remains the signer.

1. **Go to** your Agent dashboard → **Wallets → Polymarket** tab
2. If the Polymarket row shows funds on the agent wallet, click **Move funds to deposit wallet**
3. **Wait ~1–2 minutes** — Simmer submits one relayed Deposit Wallet batch on your behalf:
   - optional USDC.e approve from the Deposit Wallet to the Onramp, only if the allowance is missing
   - 1 wrap (USDC.e → pUSD), with pUSD minted back to the Deposit Wallet

   No wallet popups, no extra clicks.
4. **Deposit Wallet ready.** Your Deposit Wallet remains your V2 trading address. Accounts that already have a Deposit Wallet continue using the existing one.
5. **Your balance updates** — USDC.e drops to $0, pUSD appears with your migrated amount in the Deposit Wallet

For Deposit Wallet users, the relayer pays gas for this batch. You do **not** need POL to move or wrap these funds. **Do not send POL to your Deposit Wallet — it will be permanently stuck.**

If the Polymarket row says **Needs approval**, use the activation action shown there. Simmer bundles the Deposit Wallet deployment or exchange approvals into a single batch; managed-wallet users do not sign browser wallet popups and do not need POL for this activation.
...
**Stage 1 — Move or wrap USDC.e to pUSD:**
...
3. Use the action shown in the Polymarket wallet row:
   - **Deposit Wallet users:** click **Move funds to deposit wallet** and sign one Deposit Wallet batch. The batch contains the wrap call and, if needed, an approve call. The relayer pays gas.
   - **Legacy direct-EOA users:** click the wrap action and sign the direct `approve` and `wrap` transactions in your browser wallet. Your EOA pays Polygon gas.

**Stage 2 — Deposit Wallet:**

5. If the dashboard shows an upgrade action, click it and sign the deploy request in your wallet. This deploys your personal Deposit Wallet smart contract on Polygon. If you already have a Deposit Wallet, the dashboard uses the existing one. Once deployed, all V2 trades route through your Deposit Wallet.

**Stage 3 — Activate V2 trading:**

6. If the Polymarket row says **Needs approval**, click the activation action.
7. **Deposit Wallet users:** sign one Deposit Wallet approval batch. **Legacy direct-EOA users:** sign the individual approval transactions shown by your wallet.
8. The Polymarket row changes to **Trading** when pUSD is available and approvals are set.

Total for the current Deposit Wallet path: one signature for the move/wrap batch, plus one signature if activation approvals are still missing. No POL is needed for these relayed Deposit Wallet batches. Legacy direct-EOA accounts still use the older direct transaction path and need POL on the EOA for gas.

**If you migrated under the old wrap-only flow**, your wallet may have pUSD but missing V2 approvals. Open the Polymarket wallet row and run the activation action if it says **Needs approval**.
...
<Steps>
  <Step title="April 25-27, 2026 — Pre-cutover window">
    V1 is still live. External-wallet users can optionally pre-migrate via [polymarket.com](https://polymarket.com)'s gasless wrap flow today. **Two things worth doing now:** (1) if you have open Polymarket V1 limit orders, cancel them — the V1 order book gets wiped at cutover; (2) if you are a legacy direct-EOA user, make sure your EOA has at least **0.1 POL** for migration gas. Deposit Wallet users do not need POL for Simmer's relayed batches.
  </Step>
  ...
  <Step title="April 28, 2026 ~12:05 UTC — V2 trading live on Simmer">
    Simmer enables V2 order routing. The wrap is user-triggered from the Polymarket wallet section (no background cron): Deposit Wallet users authorize a relayed batch that wraps USDC.e into pUSD on the Deposit Wallet, while legacy direct-EOA users use direct approve + wrap transactions.
  </Step>
  <Step title="Ongoing">
...
<AccordionGroup>
  ...
  <Accordion title="Will I lose any money in the migration?">
    No. The migration is a 1:1 token swap. Simmer and Polymarket take no cut. Deposit Wallet users do not pay gas for the relayed move/wrap batch. Legacy direct-EOA users still pay Polygon gas for direct approve + wrap transactions; we recommend keeping at least 0.1 POL on that EOA if the dashboard asks for gas.
  </Accordion>

  <Accordion title="Do I need POL (gas) to migrate?">
    Usually no. Deposit Wallet users do not need POL for Simmer's current move/wrap or activation batches because Polymarket's relayer pays gas. That is the default path for managed-wallet users and the standard path for external users with a Deposit Wallet.

    Legacy direct-EOA users are the exception: direct approve + wrap transactions still spend POL from the EOA. If you are on that older path and the dashboard asks for gas, keep at least **0.1 POL** on the EOA shown in the dashboard before retrying.

    **Important: do not send POL to your Deposit Wallet.** The Deposit Wallet has no native-asset withdrawal path — POL sent there cannot be moved out. POL only belongs on the agent wallet (EOA) when the dashboard explicitly asks a legacy direct-EOA account for gas. After migration, V2 trades are gasless via Polymarket's relayer.
  </Accordion>
...
You're trying to trade V2 but your wallet still holds USDC.e, not pUSD. **Fix:** open the dashboard **Wallets → Polymarket** section and use the available move or wrap action. After the migration completes, your order should succeed.
...
**Fix:** Go to your dashboard **Wallets** tab and use the Deposit Wallet upgrade or activation action shown in the Polymarket row. Simmer will deploy your personal Deposit Wallet smart contract when needed. This is a one-time step. After the upgrade completes, run the activation action if the Polymarket row still says **Needs approval**, and your next trade should go through.
...
You are on the legacy direct-EOA wrap path. Deposit Wallet users should not hit this error because the relayer pays gas for Deposit Wallet batches. **Fix:** the dashboard error message displays the EOA that needs gas; send a small amount of POL there from an exchange (POL is Polygon's native gas token, available on most major exchanges), then retry the wrap action. Do not send POL to your Deposit Wallet.

### Migration button stuck / transaction failing

- If the dashboard asks for POL, check the displayed EOA has POL for gas. Deposit Wallet batches do not require user POL.
