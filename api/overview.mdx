...
Sell orders are never blocked by the daily trade cap (you can always exit), but completed trades of either side count toward the daily total. Free users can raise the cap up to 1,000 themselves — no upgrade required. Configure via `PATCH /api/sdk/user/settings`.
...
| `max_trades_per_day` | int | Daily trade limit across all venues. Sells never blocked, but count toward the total. Free: max 1,000. Pro: max 5,000. |
