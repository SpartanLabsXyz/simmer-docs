---
title: Simmer REST API — auth, base URL, and rate limits
description: Reference for the Simmer REST API — base URL, bearer token authentication with agent keys, response format, error codes, and per-tier rate limits.
icon: code
---
...
| `max_trades_per_day` | int | Daily buy limit across all venues — sells and redemptions are exempt. Free: max 1,000. Pro: max 5,000. |
...
At \$0.005/call, **\$5 gets you 1,000 calls**. Send USDC on Base to your wallet address, or bridge from other chains via [Base bridges](https://docs.base.org/base-chain/network-information/ecosystem-bridges).
...
```python
markets = client.get_markets(status="active", include="resolution_criteria")
for m in markets:
    criteria = m.resolution_criteria or ""
