...
# Loop mode (long-running, background in your runtime)
# Shell:
nohup python copytrading_trader.py --reactor > reactor.log 2>&1 &
# OpenClaw:  exec command="python3 copytrading_trader.py --reactor" background=true timeout=7200
# Hermes:    exec command="python3 copytrading_trader.py --reactor" background=true
```

Loop mode polls every 2s and runs until stopped. It's designed to run as a persistent background process in your agent's runtime.
