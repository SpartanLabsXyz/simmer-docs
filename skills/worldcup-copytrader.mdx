...
<Warning>
**Retired.** This skill was deprecated on 2026-07-24 and the leader-set endpoint it depended on (`GET /api/sdk/wc/copy-leaders`) is no longer served. Existing installs will not fetch new leaders. This page is kept for reference.

For general copytrading, use [`polymarket-copytrading`](/pro/reactor#signal-type-catalog), which takes a wallet list you configure and is not World Cup scoped.
</Warning>

The `polymarket-worldcup-copytrader` skill copied the top World Cup traders on Polymarket using Simmer's auto-curated leader set. Unlike [`polymarket-copytrading`](/pro/reactor#signal-type-catalog), there is no wallet list to configure — Simmer's daily curation job screened the top WC traders for *copyability* (slippage-adjusted copy P&L via PolyNode) and served the qualified set through the `GET /api/sdk/wc/copy-leaders` endpoint, which has since been withdrawn.
...
1. Fetched the daily-curated World Cup leader set from `GET /api/sdk/wc/copy-leaders` (endpoint withdrawn).
...
3. **Set your Simmer API key** (from [simmer.markets/dashboard](https://simmer.markets/dashboard?ref=docs&utm_campaign=docs), SDK tab):
