...
The `polymarket-worldcup-copytrader` skill copies the top World Cup traders on Polymarket using Simmer's auto-curated leader set. Unlike [`polymarket-copytrading`](/pro/reactor#the-first-reactor-skill-polymarket-copytrading), there is no wallet list to configure — Simmer's daily curation job screens the top WC traders for *copyability* (slippage-adjusted copy P&L via PolyNode) and serves the qualified set through [`GET /api/sdk/wc/copy-leaders`](/api-reference/wc-copy-leaders).
...
2. **Install the Simmer SDK** (0.17.27 or newer):
...
Validate on `$SIM` before switching to `--venue polymarket`. See [Venues](/venues#paper-trading-strategy) for the paper-to-real graduation path.
