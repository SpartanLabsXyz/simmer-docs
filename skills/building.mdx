...
<Warning>
**Declare every env var your SKILL.md teaches users to set.** ClawHub's moderation scanner (OpenClaw) flags skills where the `SKILL.md` body references credentials that aren't declared in `clawhub.json` `requires.env` — it treats the mismatch as a "hidden credential" signal and will block non-interactive installs.

Common env vars for Simmer skills:

| Env var | When to declare |
|---------|-----------------|
| `SIMMER_API_KEY` | Always — every skill uses it |
| `WALLET_PRIVATE_KEY` | Declare if your SKILL.md mentions external-wallet trading or self-custody signing |
| `EVM_PRIVATE_KEY` | Declare if your skill signs EVM transactions directly (e.g. x402 payments) |
| `SOLANA_PRIVATE_KEY` | Declare if your skill signs Kalshi/Solana transactions |
| `POLYGON_RPC_URL` | Declare if your skill uses a custom Polygon RPC endpoint |

If you don't use an env var, don't declare it. If you mention it in SKILL.md instructions, you must declare it.
</Warning>
