...
## Linking your content (featured\_in)

Skills that have been discussed externally — a tweet, blog post, YouTube video — can link back from their detail page on `simmer.markets/skills`. Add a `featured_in` array under `metadata.simmer` in your SKILL.md frontmatter:
...
The Simmer registry infers an icon from each URL's hostname (Twitter/X, YouTube, or a generic external-link icon) and renders icon-only links on the skill detail modal. Visitors hover each icon to see the destination hostname before clicking.

Rules:
- URLs must start with `https://` or `http://` — other schemes are silently dropped
- Up to 10 URLs per skill; extras are truncated
- Re-publish to update (sync picks up the new frontmatter within 6 hours)
- Removal is **additive-only** in v1: clearing `featured_in` from your SKILL.md does not remove existing entries from the registry. To delete a link, email `simmer@agentmail.to` with your skill slug and the URL to remove.
