...
This release also published two new ClawHub skills from the SDK bundle. [`polymarket-worldcup-copytrader`](/skills/worldcup-copytrader) follows curated World Cup leader wallets through Simmer's copy-leader feed and can place Polymarket trades automatically when run live. Install it with `clawhub install simmer/polymarket-worldcup-copytrader`, and run it in sim or dry-run mode first before allowing real-money execution.

`polymarket-fast-scaler` v1.0.0 is now published as a BTC fast-market strategy for agents that want a packaged momentum/conviction-scaling template instead of hand-rolled loop code. It was live-tested for 48 hours on a real Polymarket wallet before publication. Install it with `clawhub install simmer/polymarket-fast-scaler`.
