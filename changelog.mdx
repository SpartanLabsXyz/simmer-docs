...
description: "Release notes for the Simmer platform, Python SDK, and agent-facing REST APIs — new features, breaking changes, and deprecations by date."
...
## 2026-09-09 — `simmer-sdk` 0.25.4: replay backtests now reject unsupported tag filters

Released [`simmer-sdk` 0.25.4](https://pypi.org/project/simmer-sdk/0.25.4/) on PyPI. The backtest replay server now rejects `tags=` on `get_markets()` instead of silently ignoring it. This was the last discovery filter the replay engine did not implement: a backtest narrowed to a tag slice could accidentally run against the full market universe and report P&L for markets the strategy never meant to trade.

Passing `tags` now raises a clear error naming the unsupported parameter. Omitting it behaves exactly as before.

`pip install --upgrade simmer-sdk`

---

## 2026-09-08 — `simmer-sdk` 0.25.3: replay backtests now reject unsupported venue/status/category filters

Released [`simmer-sdk` 0.25.3](https://pypi.org/project/simmer-sdk/0.25.3/) on PyPI. The backtest replay server now rejects `venue=` and `status=` on `get_markets()`, and `category=` on the importable-market listing, instead of returning an unfiltered result set. Those filters were accepted by the SDK surface but were never implemented inside replay, so a backtest asking for one venue or market state could produce results from markets outside the requested slice.

The fix is intentionally fail-closed: passing one of those unsupported filters raises a clear error naming the parameter. Calls that omit them keep the same market universe, sort order, and default behavior.

`pip install --upgrade simmer-sdk`

---

## 2026-09-07 — `simmer-sdk` 0.25.2: Kalshi dry runs no longer submit real orders

Released [`simmer-sdk` 0.25.2](https://pypi.org/project/simmer-sdk/0.25.2/) on PyPI. `trade(dry_run=True, venue="kalshi")` no longer places a real order. The Kalshi bring-your-own-wallet path did not have a true preview mode: the call fetched a quote, signed a Solana transaction with `SOLANA_PRIVATE_KEY`, and submitted it to the venue even though `dry_run=True` promised a no-op.

Until Kalshi preview pricing exists, `dry_run=True` with `venue="kalshi"` now returns `success=False` with an error that names the missing preview path, before any quote, signing, or submit call. `dry_run` on sim and Polymarket is unchanged.

`pip install --upgrade simmer-sdk`

---

## 2026-09-06 — `simmer-sdk` 0.25.1: paper dry runs and paper/live guards fixed

Released [`simmer-sdk` 0.25.1](https://pypi.org/project/simmer-sdk/0.25.1/) on PyPI. Paper `trade(dry_run=True)` no longer mutates the in-memory paper portfolio. On a `live=False` client, a preview followed by the real paper trade on the same market could double-book shares and cost basis, causing paper expectancy to drift from the strategy's actual trade path. The dry run still returns the same `TradeResult`; it just leaves the book unchanged.

This release also corrects two paper/live safety signals. `venue="polymarket"` with `live=False` no longer prints "LIVE trading with real funds" merely because it is using Polymarket prices. And direct `client.hyperliquid` write methods now honor `live=False` and `readonly()`: reads still work, but `place_order`, `cancel_order`, and `approve_agent` raise instead of submitting from a paper or readonly client.

`pip install --upgrade simmer-sdk`

---

## 2026-09-10 — `simmer-sdk` 0.25.5: empty market searches explain themselves, and tradeability lands on Market objects

Released [`simmer-sdk` 0.25.5](https://pypi.org/project/simmer-sdk/0.25.5/) on PyPI. `get_markets()` now returns a `MarketList` — a `list` subclass that carries `empty_reason`, `hint`, and `matched_before_tradeable_filter` alongside the market entries. An agent that gets an empty list can now tell "no such markets exist" from "matching markets exist, but the default tradeability filter dropped them", then follow the embedded hint and retry with `tradeable_only=False` when that is the right move.

`Market` objects now expose `sim_tradeable` and `is_orderbook_open`, which the API was already returning but the SDK parser was dropping. Existing callers do not need code changes: iteration, `len`, indexing, slicing, `== []`, and `isinstance(x, list)` all behave the same on `MarketList`; the new fields are additive, and omitting `tradeable_only` keeps the same default behavior.

`pip install --upgrade simmer-sdk`

---

## 2026-09-06 — `simmer-sdk` 0.25.0: backtests now charge fees by default

Released [`simmer-sdk` 0.25.0](https://pypi.org/project/simmer-sdk/0.25.0/) on PyPI. `simmer backtest` now charges fees, and your numbers will change. `--fee-rate` follows the replay engine's base rate (currently `0.05`) instead of `0`, applied as `notional × rate × (1 - price)`; resting limit orders that fill as makers pay nothing. Pass `--fee-rate 0` for the previous fee-free behaviour.

Two consequences worth planning for. Every run now carries cost drag, so a strategy that beat its baselines before may not. And every `config_hash` changed, so re-running a saved backtest will not reproduce its stored report — that is expected, not a bug.

Reports now say what they charged: `summary.fee_rate`, `summary.fees_paid`, `reproducibility.fee_rate`, and a `fee` on every entry in `fills[]`. Baselines pay the same fee as the strategy's own fills and are derived from executed fills rather than logged decisions, so a limit order that never crossed no longer counts as a buy-and-hold entry.

A worthless position can also be closed again: `sell()` briefly rejected a price of exactly 0 — the case where YES prints 1.0 and a NO position is worth nothing — leaving it on the book to be marked and settled instead of exited.

`pip install --upgrade simmer-sdk`

---

## 2026-09-05 — `simmer-sdk` 0.24.7: tag search, dry-run cache, and sell guidance fixes

Released [`simmer-sdk` 0.24.7](https://pypi.org/project/simmer-sdk/0.24.7/) on PyPI. `find_markets()` was silently discarding tag-only matches: the server's `q` filter matches a search term against the market question OR its tags, but `find_markets()` re-filtered those results on question text alone, so `find_markets("weather")` returned nothing even though `get_markets(q="weather")` returned a full page of markets titled like "Austin 82-83F on Sep 7" that only carried `weather` as a tag. The server's matches are now returned as-is.

`trade(dry_run=True)` no longer blocks the real trade that follows it. A dry run was updating the client-side held-markets cache exactly as a real fill would, so the next real buy on that market was refused with "Already hold position on this market. Pass allow_rebuy=True to override." — even though `get_positions()` was empty and the server held nothing. This is the exact preview-then-place sequence shown in our own docs, so anyone following that pattern hit it directly. A dry run now leaves the cache untouched on the sim and Polymarket paths. This 0.24.7 release did not cover the Kalshi bring-your-own-wallet path: SDKs earlier than 0.25.2 could ignore `dry_run` and submit a real order (SIM-5041). In `simmer-sdk` 0.25.2 and later, Kalshi dry runs fail closed with `success=False` and no order placed. Preview Kalshi trades with `POST /api/sdk/trade/kalshi/quote` and simply don't submit — nothing reaches the venue until you sign the returned transaction.

The `simmer` skill's exit guidance (skill 1.25.1) has also been corrected. It previously described selling as `client.trade(side="no", ...)`, which the API treats as a buy of the opposite side and rejects with "amount required for buy orders." To sell, use `action="sell"` with a share count — `side` names the side you hold, not the action you're taking.

`pip install --upgrade simmer-sdk`

---

## 2026-08-31 — Optional `idempotency_key` on SDK trade requests

`POST /api/sdk/trade` now accepts an optional `idempotency_key` string so retrying a request does not double-submit the same live order. The first completed request for a key is cached for 24 hours; replaying the same key returns that cached response instead of submitting to the venue again. Concurrent duplicate submissions while the first request is still in flight return a retryable 503, so agents can back off rather than race themselves.

If Redis is unavailable and you provide an idempotency key, the endpoint fails closed with 503 rather than submitting an order it cannot deduplicate. Omitting `idempotency_key` preserves the previous behavior. Use this when your runtime may retry after timeouts or process restarts and you need one logical trade intent to produce at most one venue submission.

---

## 2026-08-23 — `simmer-sdk` 0.24.6: dry-run pricing and fast-market status filters

Released [`simmer-sdk` 0.24.6](https://pypi.org/project/simmer-sdk/0.24.6/) on PyPI. `client.trade(dry_run=True)` now validates and prices a trade without executing it, so an agent can inspect the fill count and cost estimate before committing size. On a dry run the SDK does not sign an order locally, because the server never submits it.

Two caveats matter for automation. A dry run is not a permission check: account trading-limit enforcement is skipped for previews, so the same request can still be rejected live for a daily buy cap or cooldown. On Polymarket, the preview prices from the market's external price rather than the executable order book, so neg-risk cost estimates can differ from the live fill.

This release also adds `market_status=` to `client.get_fast_markets()`, matching the endpoint's existing `live` / `upcoming` filter. Leave it unset to see both live and not-yet-open fast markets for an asset in one call.

`pip install --upgrade simmer-sdk`

---

## 2026-08-19 — `simmer-sdk` 0.24.5: Polymarket import picks up env wallets

Released [`simmer-sdk` 0.24.5](https://pypi.org/project/simmer-sdk/0.24.5/) on PyPI. `client.import_polymarket_wallet()` now adopts `OWS_WALLET` or `WALLET_PRIVATE_KEY` from the environment when the client itself has no signer. This fixes the documented `SimmerClient.from_env()` import snippet for sim-default clients: `from_env()` intentionally ignores live wallet keys unless you ask for a live venue, but calling `import_polymarket_wallet()` is explicit intent to use the local wallet for the import flow.

If you saw `private_key or ows_wallet required` while following the Polymarket import guide with the key already in your environment, upgrade and retry the same call.

`pip install --upgrade simmer-sdk`

---

## 2026-08-19 — `simmer-sdk` 0.24.4: Polymarket wallet import and OWS per-agent deprecation

Released [`simmer-sdk` 0.24.4](https://pypi.org/project/simmer-sdk/0.24.4/) on PyPI. The new `client.import_polymarket_wallet()` helper turns a Polymarket-exported wallet into a trading-ready Simmer setup in one call: it proves wallet ownership, links the wallet to your Simmer account, adopts the deposit wallet Polymarket already deployed when one exists, tops up the extended spender approvals gaslessly, and reports the resulting balance. The same helper also works for a self-generated key; in that case Simmer deploys a fresh deposit wallet instead of adopting an existing one. `client.link_wallet()` also gains `confirm_orphan_deposit_wallet` for explicit wallet switches on accounts that already have an active deposit wallet, plus `source` for onboarding-path attribution. Leave `source` unset unless you are intentionally building a tracked onboarding wrapper.

Breaking change: this release closes the entrance for new OWS per-agent registrations. `register_agent_wallet(ows_wallet_name=...)` now emits a `DeprecationWarning`, and the server rejects new OWS per-agent registrations. Existing OWS per-agent wallets keep working: `update_agent_wallet_creds(ows_wallet_name=...)` remains supported for credential refreshes. OWS itself is not deprecated as a local key store — `OWS_WALLET` continues to work for the standard external-wallet path with `link_wallet()` and normal trade signing.

`pip install --upgrade simmer-sdk`

---

## 2026-08-18 — `simmer-sdk` 0.24.3: go-live steps on sim-trade responses

Released [`simmer-sdk` 0.24.3](https://pypi.org/project/simmer-sdk/0.24.3/) on PyPI. Successful sim trades can now carry a `go_live` block: the server attaches it when your agent crosses an activity milestone (10, 50, 100, or 250 sim trades) and the account has never traded on a real venue. The block lists the exact steps to go live, split by actor — agent steps you can run yourself (enabling real trading via `PATCH /api/sdk/settings`, running `activate_polymarket_dw()` where approvals are missing) and owner steps your human must do at the dashboard (funding the wallet). `TradeResult.go_live` exposes it, and `include_hints=True` also lands the steps in `next_steps`. Older SDK versions still see a one-line summary in `warnings`. Purely informational: nothing is enabled or transacted on your behalf.

`pip install --upgrade simmer-sdk`

---

## 2026-08-09 — `simmer-mcp` v3.4.9: MCP tool annotations for read-only clients

Released [`simmer-mcp` v3.4.9](https://www.npmjs.com/package/simmer-mcp/v/3.4.9) on npm. All 21 MCP tools now emit `readOnlyHint` and `destructiveHint` annotations from the existing `mutates` metadata. MCP clients that enforce plan mode or read-only mode were previously falling back to name-based heuristics; because Simmer tools use the `simmer_` prefix rather than `get` or `list`, five safe read tools were misclassified as writes and blocked.

The write-capable paths remain gated. Tools that can place live orders or modify the local experiment workspace carry explicit destructive annotations, while simulated backtests and ordinary read tools are annotated as read-only. Upgrade with `npm install -g simmer-mcp@latest` or use `npx -y simmer-mcp@latest` in your MCP config to run the current npm release on launch.

---

## 2026-08-07 — `simmer-sdk` 0.24.2: deposit-wallet approvals warning fixed

Released [`simmer-sdk` 0.24.2](https://pypi.org/project/simmer-sdk/0.24.2/) on PyPI. The SDK no longer fires a spurious approvals warning every session for healthy deposit-wallet users. `_warn_approvals_once()` was checking the signer EOA instead of the deposit wallet, so deposit-wallet accounts looked unapproved even when their live trading approvals were already set.

The warning now checks the deposit wallet when the client is using one, and the remediation text points that cohort to `activate_polymarket_dw()` instead of `set_approvals()`, which cannot fix deposit-wallet approvals. If you saw repeated approval warnings on a deposit-wallet setup, upgrade and restart the agent.

`pip install --upgrade simmer-sdk`

---

## 2026-08-06 — `simmer-sdk` 0.24.1: NegRiskAdapter approval restored for neg-risk markets

Released [`simmer-sdk` 0.24.1](https://pypi.org/project/simmer-sdk/0.24.1/) on PyPI. A fix to `set_approvals()` restores the NegRiskAdapter to the V2 trading spender set. After Polymarket's 2026-07-18 adapter retirement, a prior SDK version incorrectly removed the NegRiskAdapter pUSD allowance from the approval set — wallets that ran `set_approvals()` without it were silently rejected on neg-risk market orders. The 12-transaction V2 approval set is now fully restored. If you ran approvals with 0.23.x or 0.24.0, run `client.set_approvals()` again after upgrading to pick up the missing grant.

`pip install --upgrade simmer-sdk`

---

## 2026-07-22 — `simmer-sdk` 0.24.0: token-light read outputs and read-only clients

Released [`simmer-sdk` 0.24.0](https://pypi.org/project/simmer-sdk/0.24.0/) on PyPI. `client.get_markets()`, `client.get_positions()`, and `client.get_trades()` now accept `response_mode="summary"` or `"compact"` for smaller, agent-friendly responses. Use `response_mode="toon"` when you want a TOON string for token-light transcripts, and add `include_hints=True` when the response should include suggested next calls. Trade failures can now also carry structured `TradeResult.error_code`, `TradeResult.error_hint`, and `TradeResult.next_steps` fields without changing existing success/error semantics.

This release also adds `SimmerClient.readonly()` for validation, preflight, and status scripts. It preserves normal venue selection for read endpoints, but skips the live Polymarket constructor's init-time risk-exit processing and guards order/risk-exit submission methods, so API-key checks and account probes can inspect state without submitting orders.
...
## 2026-07-15 — Wallet-dedication safety guard for Elite users

Elite users who click "Dedicate a wallet" immediately after creating an agent now see a confirmation step when the account-primary wallet holds at least $0.50 across pUSD, USDC.e, USDT, or native USDC. The step summarizes the detected balance and explains that dedicating the wallet will move those funds into the new agent's wallet before offering **Go back** or **Proceed**.

Users with an unfunded or undetectable account-primary wallet continue directly without interruption. This keeps the fast path for clean setups while adding an explicit checkpoint for the case where a user might otherwise strand funds by dedicating the primary wallet to a new agent too quickly.

---

## 2026-07-15 — Wallet-link endpoints now enforce their documented rate limits

The wallet-link challenge and wallet-link submission endpoints have always documented retry limits of 5 requests/hour and 3 requests/day respectively. Those limits are now enforced by Simmer's platform rate limiter.

This closes an abuse surface in the wallet-ownership-proof flow without changing normal usage. Agents and users operating within the documented limits should see no behavior change; excessive retries now receive the same rate-limit handling as other protected SDK and dashboard endpoints.

---

## 2026-07-13 — `simmer-sdk` 0.22.3: `client.get_outcomes()` for settlement-accurate skill metrics

Released [`simmer-sdk` 0.22.3](https://pypi.org/project/simmer-sdk/0.22.3/) on PyPI. New `client.get_outcomes()` method returns settlement-accurate skill outcome summaries for autoresearch metric verification and skill-health signals.
...
## 2026-07-10 — `simmer-sdk` 0.22.2: server-side filters for `get_trades()`

Released [`simmer-sdk` 0.22.2](https://pypi.org/project/simmer-sdk/0.22.2/) on PyPI. `client.get_trades()` now accepts `market_id`, `since`, `until`, and `include_failed` keyword arguments, and forwards them to `GET /api/sdk/trades` so the backend filters before paging. Existing calls keep the 0.22.1 behavior when the new filters are omitted.

Use `market_id` to inspect one market's history, `since` / `until` with ISO-8601 timestamps for time-bounded reconciliation, and `include_failed=True` when you need failed, cancelled, or expired real-trade attempts in the response. Failed rows include `failure_category` and a sanitized `failure_reason`; raw venue/API error payloads are not exposed.
