...
The write-capable paths remain gated. Tools that can place live orders or modify the local experiment workspace carry explicit destructive annotations, while simulated backtests and ordinary read tools are annotated as read-only. Upgrade with `npm install -g simmer-mcp` or keep using `npx -y simmer-mcp` to pick up the latest package on launch.
