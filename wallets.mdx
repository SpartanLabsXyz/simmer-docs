...
| Custody model | Simmer-managed signing | Self-custody |
| Setup | Just an API key | `WALLET_PRIVATE_KEY` env var + dashboard activation |
| Best for | Fastest setup, no key management, server-side automation | Local signing, on-chain transparency, full control |
