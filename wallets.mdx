...
**Funding:** Send USDC.e to your **agent wallet** address (visible in the dashboard **Wallets** tab). The atomic "Move to trading" flow wraps it to pUSD and transfers it to your Deposit Wallet in one batched transaction. **Do not send funds directly to the Deposit Wallet address** — its only withdrawal paths are USDC.e and pUSD, so any other asset (POL, ETH, native USDC, wrong-chain) sent there cannot be moved out. See the [V2 Migration page](/v2-migration) for the full funding warning + recovery rules.
