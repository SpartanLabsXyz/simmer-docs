...
For **\$SIM**, realized and unrealized come from `compute_sdk_agent_sim_pnl_async` (cash delta + open-position mark-to-market). For **Polymarket**, realized comes from the Simmer DB cache, derived from `real_trades` plus settlement data; unrealized is derived as `pnl − realized_pnl`. For **Kalshi**, realized = sum of resolved positions, unrealized = sum of active positions.
