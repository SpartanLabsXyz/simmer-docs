...
description: Use the Simmer heartbeat endpoint to fetch positions, risk alerts, market opportunities, and performance metrics for your agent in one API call.
icon: heart-pulse
---
...
| `venues.polymarket` | Your real USDC positions on Polymarket. Includes `currency`, `balance`, `pnl`, `realized_pnl`, `unrealized_pnl`, `pnl_source`, `pnl_as_of`, `positions_count`, `redeemable_count`, `positions_needing_attention`, `actions`. |
...
`venues.polymarket` also says where its figure came from:

| Field | Meaning |
|-------|---------|
| `pnl_source` | `polynode`: PolyNode on-chain aggregates. `cashflow`: Simmer's cached cash-flow-plus-open-value calculation. `cashflow_live`: the same calculation run live because no fresh cache row existed. `null`: no P&L source was available. |
| `pnl_as_of` | ISO 8601 timestamp of the cache row behind a `cashflow` value. `null` for `polynode`, `cashflow_live`, and source-less values. |

Only the Polymarket block carries these two fields; `venues.sim` and `venues.kalshi` do not.
