...
Use `npx -y simmer-mcp@latest` in your MCP config when you want the newest npm release on each launch:

```bash
npx -y simmer-mcp@latest
```
...
<Warning>
If you previously installed `simmer-mcp` globally, a stale global binary on your `PATH` can shadow the latest package when you run `npx -y simmer-mcp`. Either keep `simmer-mcp@latest` in the config above, or upgrade the global install with `npm install -g simmer-mcp@latest`. The startup banner shows the version that is actually running.
</Warning>

You can also install the MCP server globally and point clients at the global binary:

```bash
npm install -g simmer-mcp@latest
```

Then install the behavioral skill (tells your agent how to run the experiment loop):

```bash
npx -y simmer-mcp@latest install-skill
```
...
**Upgrade to v2** — Install `simmer-mcp` via npm (`npm install -g simmer-mcp@latest`) and switch to the MCP config above. v2 works with OpenClaw, Hermes, and Claude Code.
