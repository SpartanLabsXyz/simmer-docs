---
title: "Autoresearch: autonomous skill optimization for agents"
description: "Autoresearch is a Simmer Pro plugin that mutates your trading skill config, measures P&L on each variant, and keeps the parameters that perform best."
icon: flask
---
...
- **Check confidence scores.** ≥2× noise floor = improvement is likely real. <1× = within noise. 1-2× = marginal, re-run to confirm.
...
Your agent manages its own session state using the SKILL.md behavioral instructions (installed via `npx simmer-autoresearch install-skill`). There is no `/autoresearch` command interface — the agent drives the loop autonomously.
