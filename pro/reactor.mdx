...
The former World Cup `shock_ladder` flow and `polymarket-soccer-shock-ladder` skill are deprecated. They are not current install or run targets, and their old arming endpoint is not a live setup path.
...
<Note>
  **This is no longer a self-serve Reactor signal.** The 2026 tournament produced 10,401 signals between 12 June and 18 July 2026, but new opt-ins are closed and `/api/sdk/shock-ladder/config` is not served as a live customer arming endpoint.
