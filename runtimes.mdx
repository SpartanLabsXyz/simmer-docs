---
title: Runtimes
description: Install and run Simmer trading skills from any agent runtime that supports the agentskills.io standard.
...
| **Grok Bot** | 🟡 Install syntax pending | Install into `workflows/` — see [shared-computer warning below](#grok-bot) before storing a real-money key. |
...
Install the skill folder into Grok Bot's `workflows/` directory — it's picked up the same way as any agentskills.io-compliant skill.

<Warning>
**Grok Bot's cloud computer is shared across every bot on your account.** Any credential stored there — including a Simmer API key or a wallet's private key — is readable by every other bot running on that same shared computer, not just the one you configured. Register a separate Simmer agent for Grok Bot and leave that agent unclaimed (sim/paper only) rather than storing a real-money key on shared infrastructure.
</Warning>
