...
|---|---|---|
| **Primary agent (external)** | Browser-signable | Manual trades placed from the dashboard, signed via the connected browser wallet. |
| **Primary agent (managed)** | Server-signed | Manual trades placed from the dashboard, signed server-side with the encrypted key. |
