---
title: Backtesting
description: Run the simmer-sdk backtest CLI to test a trading skill on historical prediction-market data — configure fills, fees, and slippage before going live.
...
── backtest summary ─────────────────────────────────────────
  ...
  ℹ  known leaks: bundle-local wall clock (e.g. daily-spend reset cadence)
...
| `--fee-rate` | Base taker fee rate, charged as `notional x rate x (1 - price)`. Defaults to the engine's rate (currently `0.05`); pass `0` to run fee-free. |
...
- **`summary`** — pnl, hit rate, max drawdown, trades, decisions, settlements, ticks,
  plus `fee_rate` (the base rate charged) and `fees_paid` (the total).
- **`baselines`** — the same entries/notionals under *buy-and-hold-YES* and a
  seeded *random* side rule, so you can tell skill from luck.
- **`decisions`** / **`fills`** / **`equity_curve`** — the full per-tick trace. Each
  fill carries its own `fee`.
...
- **`reproducibility.config_hash`** — a deterministic hash of the run inputs.
  ...
  results. `reproducibility.fee_rate` records the fee assumption behind the P&L.
