...
```bash
pip install -U simmer-sdk   # examples on this page verified against 0.20.3
```

<Note>
Run `pip install -U simmer-sdk` to get the latest version. These examples were verified against `simmer-sdk` 0.20.3. Versions before 0.20.0 are missing several `get_markets` keyword filters (`q`, `venue`, `sort`, `tags`) and will raise `TypeError`.
</Note>
