---
title: Trading Guide
description: End-to-end trading workflow on Simmer — discover prediction markets, place buy and sell orders, manage open positions, and exit with stop-loss or take-profit.
icon: chart-line
---
...
print(f"Would buy {preview.shares_bought} shares for {preview.cost}")

# Happy with the size? Place it with the same arguments, minus dry_run.
```
...
- `fee_rate_bps` — the fee rate embedded in the signed order, in basis points (SDK 0.21.0). On Polymarket V2 it is always `0`, which means "unknown at placement time", **not** "free" — the taker fee is set at match time. Estimate it from [Polymarket's published formula](https://docs.polymarket.com/trading/fees); see [venue fees](/faq#fees) for the derivation.
...
if result.fill_status == "filled":
    ...
    # Check back later or cancel with client.cancel_order(result.order_id)
...
After a market resolves, redeem winning positions to collect your payout. For external Deposit Wallets, the SDK handles the prepare, local-sign, and submit flow through `POST /api/sdk/dw-redeem/prepare` and `POST /api/sdk/dw-redeem/submit` — see the full [Redemption guide](/redemption) for details.
...
<Tabs>
  ...
  <Tab title="curl (external Deposit Wallet)">
    ```bash
    ...
    Sign the returned `typed_data`, then submit the prepared `calls`, `nonce`, `deadline`, and `signature` to `POST /api/sdk/dw-redeem/submit`.
  </Tab>
</Tabs>
