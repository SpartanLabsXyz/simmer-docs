...
Test your trade without executing it. On **sim and Polymarket**, both the REST endpoint and `client.trade(dry_run=True)` (SDK 0.24.6+, keyword-only) validate and price the trade, returning estimated shares, cost, and fees without moving money or signing an order. Kalshi has no `dry_run` — read the warning below before previewing there. For a full session with balance tracking instead of a single preview, use `live=False` paper trading — see [Practice modes](/venues#practice-modes).

<Warning>
**`dry_run` does not work on Kalshi, by either route.** Through the Python SDK, `client.trade(venue="kalshi", dry_run=True)` reaches the Kalshi signing path before the flag is read and **places a real order** (SIM-5041). Through REST, `POST /api/sdk/trade` ignores `dry_run` on Kalshi and returns an error telling you to use the SDK — so following it lands you back on the path that executes.
