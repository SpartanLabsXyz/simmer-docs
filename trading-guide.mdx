...
Test your trade without executing it. Both the REST endpoint and `client.trade(dry_run=True)` (SDK 0.24.6+, keyword-only) validate and price the trade, returning estimated shares, cost, and fees without moving money or signing an order. For a full session with balance tracking instead of a single preview, use `live=False` paper trading — see [Practice modes](/venues#practice-modes).

<Warning>
**`dry_run` is not honoured on Kalshi through the Python SDK.** `client.trade(venue="kalshi", dry_run=True)` reaches the Kalshi signing path before the flag is read and **places a real order**. Tracked as SIM-5041. Until it lands, preview Kalshi trades through the REST endpoint, which honours `dry_run` on every venue.
</Warning>

A dry run is also **not a permission check**. The server skips account trading-limit enforcement on previews, so a trade that dry-runs clean can still be rejected live for a daily buy cap, a spend cap, or a failed-trade cooldown — read `client.get_settings()` for those. On Polymarket it prices from the market's external price rather than the executable book, so neg-risk cost estimates can differ from the live fill.
