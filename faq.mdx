...
**Polymarket:** Makers pay nothing. Takers pay a fee set at match time, not at order placement. Measured across 6,812 back-filled V2 trades, the effective taker fee ran at a median of **246 basis points** (~2.5% of notional). Budget for it. The `fee_rate_bps` field on a trade response cannot tell you the number — see "How do I read the realized fee and fill price from a trade?" below.
...
<Warning>
**Do not compute net edge from `fee_rate_bps` on Polymarket.** Under V2 it is always `0` because the fee is decided at match time and is not knowable when the order is signed. The realized fee lands in the on-chain Trade event afterwards and is not yet exposed through the API. Until it is, size against a working assumption rather than zero: the only fees Simmer has measured are 6,812 back-filled V2 trades, which came in at a median **246 bps effective** at an average price of 0.522 — implying a base rate near 500 bps. `fill_price` is unaffected and remains exact.
</Warning>
</Accordion>
