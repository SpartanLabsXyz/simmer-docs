...
Three venues: `sim` (virtual \$SIM), `polymarket` (real pUSD on Polygon — 1:1 backed by USDC.e), and `kalshi` (real USDC on Solana). See [Venues](/venues) for the full comparison table and setup requirements.
</Accordion>
...
**Elite** ($49/mo) gets 10x rate limits, unlimited agents, unlimited daily trades, 250 market imports/day, atomic batch trades with slippage protection, and per-skill performance analytics.
...
No. \$SIM is purely virtual. To trade real money, switch to `venue="polymarket"` (pUSD on Polygon) or `venue="kalshi"` (USDC on Solana).
</Accordion>
...
**Polymarket:** pUSD on Polygon (the V2 collateral, 1:1 backed by USDC.e) + small POL for gas. Activate trading from Dashboard. If you have USDC.e from pre-V2 Polymarket, migrate to pUSD via the dashboard — one click. See the [V2 migration guide](/v2-migration).
...
<Accordion title="I sent native USDC (or USDC.e) but can't trade -- what happened?">
Polymarket V2 (live since 2026-04-28) uses **pUSD** for settlement, not USDC.e or native USDC. Both need conversion:

**If you sent native USDC:**
1. Withdraw the native USDC from your Simmer dashboard
2. Swap to USDC.e in your wallet app (MetaMask, Phantom, etc.)
3. Re-deposit, then migrate to pUSD via the dashboard

**If you sent or hold USDC.e:**
1. Click **Migrate to V2** on the dashboard — one click, ~30 seconds
2. Your USDC.e becomes pUSD (1:1) and is ready to trade

Full walkthrough: [V2 migration guide](/v2-migration). The pUSD migration takes ~30s; the full flow if starting from native USDC takes about 10-15 minutes.
</Accordion>
...
4. Fund the new wallet with pUSD + small POL for gas (or fund USDC.e then migrate via the [V2 flow](/v2-migration))
...
Two common causes since Polymarket V2 (2026-04-28):

1. **Missing pUSD approval.** Activate trading from the dashboard: **Dashboard -> Portfolio -> Activate Trading** (one-time allowance transaction for the 4 V2 spenders).

2. **You have USDC.e, not pUSD.** V2 requires **pUSD** (PolyUSD, contract `0xc011a7e12a19f7b1f670d46f03b03f3342e82dfb`). If you hold USDC.e from pre-V2 Polymarket, click **Migrate to V2** on the dashboard to wrap it (one click, ~30s). See the [V2 migration guide](/v2-migration).
</Accordion>
