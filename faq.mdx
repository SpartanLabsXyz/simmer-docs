...
**Polymarket:** USDC.e (bridged USDC on Polygon) + small POL for gas on your **owner wallet** — the EOA address you signed in with. Activate trading from Dashboard. **Do not send POL or other assets directly to your Trading Contract (Polymarket Deposit Wallet)** — see the warning below.
...
Polymarket's Trading Contract is a smart contract that only exists on Polygon. The same address on Base, Ethereum, Arbitrum, or any other chain is empty space, not your wallet — funds sent there cannot be recovered.
