...
| **MCP server** | `npm install -g simmer-mcp@latest` -- gives your agent direct access to docs and troubleshooting |
...
<Steps>
  <Step title="Try self-service first">
    Check the [FAQ](/faq), use the AI Assistant (chat bubble), install the MCP server (`npm install -g simmer-mcp@latest`), or call `POST /api/sdk/troubleshoot` with your error.
  </Step>
